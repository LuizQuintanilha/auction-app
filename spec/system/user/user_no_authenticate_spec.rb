...
describe 'User visit homepage' do
  context 'without authenticate' do
    ...
    it 'and view all currently batches ' do
      ...
      mouse_product.photo.attach(io: File.open(Rails.root.join('spec', 'fixtures', 'file', 'mouse_red.jpg')), filename: 'mouse_red.jpg')
      mouse_product.save
      current_batch = ProductBatch.new(admin_id: 1, code: 'ABD332211', start_date: Date.today, 
                                      deadline: Date.today, minimum_value: 800, minimal_difference: 100,
                                       created_by: luiz, approved_by: luana, start_time: Time.current, end_time: 1.hour.from_now)
